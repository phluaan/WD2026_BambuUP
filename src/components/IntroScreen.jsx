...
const BAMBOO_PALETTES = [
  ...
  { young: 'linear-gradient(90deg,#e8f5b0 0%,#ccea78 30%,#ddf590 50%,#ccea78 70%,#e8f5b0 100%)',
    mature: 'linear-gradient(90deg,#6a8a18 0%,#9abf2e 30%,#bcd948 50%,#9abf2e 70%,#6a8a18 100%)',
    mid: '#9abf2e', leaf: ['#b8d840','#9abf28','#cce050'] },
  // 1 – Lime green
  { young: 'linear-gradient(90deg,#d4f0a0 0%,#b0e060 30%,#c8ed80 50%,#b0e060 70%,#d4f0a0 100%)',
    mature: 'linear-gradient(90deg,#4e7a18 0%,#78a830 30%,#98c844 50%,#78a830 70%,#4e7a18 100%)',
    mid: '#78a830', leaf: ['#94c03a','#78a828','#acd04a'] },
  // 2 – Fresh medium green (logo centre)
  { young: 'linear-gradient(90deg,#bcedc0 0%,#88d898 30%,#a8e8b0 50%,#88d898 70%,#bcedc0 100%)',
    mature: 'linear-gradient(90deg,#1e7038 0%,#38a054 30%,#52bc6a 50%,#38a054 70%,#1e7038 100%)',
    mid: '#38a054', leaf: ['#52bc6a','#38a050','#6acd7a'] },
  // 3 – Teal green
  { young: 'linear-gradient(90deg,#b0e8d0 0%,#78d4aa 30%,#98e0be 50%,#78d4aa 70%,#b0e8d0 100%)',
    mature: 'linear-gradient(90deg,#196050 0%,#2a9070 30%,#3cac88 50%,#2a9070 70%,#196050 100%)',
    mid: '#2a9070', leaf: ['#3cac88','#2a9068','#50c09a'] },
  // 4 – Emerald (rightmost logo hue)
  { young: 'linear-gradient(90deg,#a8e4cc 0%,#70d0aa 30%,#90dec0 50%,#70d0aa 70%,#a8e4cc 100%)',
    mature: 'linear-gradient(90deg,#0e5a40 0%,#1a8a60 30%,#2aad78 50%,#1a8a60 70%,#0e5a40 100%)',
    mid: '#1a8a60', leaf: ['#2aad78','#1a8a5e','#3cc488'] },
];
...
export default function IntroScreen({ onComplete }) {
  ...
  const bambooStalks = useMemo(() => {
    ...
    return [...Array(15)].map((_, i) => {
      ...
      const joints = [...Array(numJoints)].map((_, j) => {
        ...
        if (isTopNode) {
            ...
            numBranches = isTop
              ...
              : (Math.random() > (0.3 + (1 - heightFactor) * 0.4) ? 1 : 0);
        }
        ...
        if (numBranches > 0) {
           ...
           let collision = occupiedLeafZones.some(z => Math.abs(z.x - xApprox) < 35 && Math.abs(z.y - yApprox) < 8);
           if (collision) {
               jointSideIsRight = !jointSideIsRight;
               xApprox = xOffset + (jointSideIsRight ? 25 : -25);
               collision = occupiedLeafZones.some(z => Math.abs(z.x - xApprox) < 35 && Math.abs(z.y - yApprox) < 8);
           }
        ...
        const branches = [...Array(numBranches)].map((_, b) => {
          ...
          if (isTop) {
            ...
            const baseAngle = isRight ? 25 + fanSpread * 65 : -(25 + fanSpread * 65);
            branchAngle = baseAngle + (Math.random() * 20 - 10);
          } else {
            const baseBranchAngle = isRight ? 45 : -45;
            branchAngle = baseBranchAngle + (Math.random() * 30 - 15) + (b * (isRight ? 15 : -15));
          }

          // Top canopy branches are much longer to match the drooping canopy in the reference image
          const branchLength = isTop ? 70 + Math.random() * 50 : 20 + Math.random() * 25;
      ...
      joints.forEach(joint => joint.branches.forEach(branch => branch.leaves.forEach(leaf => {
        const leafColors = palette.leaf;
        leaf.color = leafColors[Math.floor(Math.random() * leafColors.length)];
      })));

      return { id: i, isShoot, delay, height, width, rotation, xOffset, curvature, joints, buds: [], paletteIndex };
    });
  }, []);
  ...
  const stages = [
    {
      id: 'seed',
      texts: [
        "Như rừng tre vươn cao không mệt mỏi,",
        "phụ nữ BambuUP là nguồn sức mạnh của chúng ta."
      ],
      action: "Cung cấp dinh dưỡng"
    },
    {
      id: 'bloom',
      texts: [
        "Mừng ngày 8 tháng 3 —",
        "Tri ân những người phụ nữ phi thường của BambuUP! 🌸"
      ],
      action: "Ngắm nhìn sự nở rộ"
    }
  ]
  ...
  return (
    ...
    >
      ...
      {/* Ground platform */}
      <div className="absolute bottom-[4vh] w-full h-[3px] opacity-50 z-0" style={{ background: 'radial-gradient(ellipse at center, #6EE7B7 0%, transparent 80%)' }} />
      <div className="absolute bottom-0 w-full h-[4vh] z-0" style={{ background: 'linear-gradient(to bottom, rgba(249,255,252,0), rgba(16,185,129,0.15))' }} />

      {/* Visual Representation Area (Bamboo & Seed - strictly tied to bottom) */}
      <div className="absolute inset-0 pointer-events-none z-10 flex justify-center md:justify-start max-w-7xl mx-auto px-6 md:px-12" style={{ left: 0, right: 0 }}>
        <div className="relative w-full md:w-1/2 h-full">
          <div className="absolute bottom-[4vh] left-0 right-0 flex justify-center items-end" style={{ height: '0px' }}>
            <AnimatePresence>
              {stage === 0 && (
                <motion.div
                  key="seed"
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      return (
                        <motion.svg
                          ...
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                          transition={{ duration: 0.3, delay: stalk.delay * 0.4 }}
                        >
                          <motion.path
                            ...
                            style={{ filter: 'drop-shadow(1px 0 2px rgba(0,0,0,0.15))' }}
                            initial={{ pathLength: 0, stroke: '#ccea78' }}
                            animate={{ pathLength: heightMultiplier, stroke: '#4a9a2e' }}
                            transition={{ duration: 3.2, delay: stalk.delay * 0.4, ease: [0.4, 0, 0.9, 1] }}
                          />
                        </motion.svg>
                      );
                    })()}
                    ...
                    >
                      ...
                      {stalk.joints.map((joint) => {
                        ...
                        const isReached = bottomPercent <= (heightMultiplier * 100 - 1); 
                        
                        let relativePos = (bottomPercent / 100 - prevMultiplier) / (heightMultiplier - prevMultiplier || 1);
                        ...
                        const jointDelay = stalk.delay * 0.4 + (relativePos > 0 ? Math.pow(relativePos, 0.5) * 3.6 + 0.5 : 0);
                        ...
                        const curveOffsetX = stalk.curvature * 2 * t * (1 - t); // quadratic peak at t=0.5
                        ...
                        return isReached && (
                          ...
                          >
                            ...
                            <div className="absolute w-[120%] h-[3px] left-[-10%] rounded-full opacity-70 shadow-[0_1px_2px_rgba(0,0,0,0.25)]" style={{ background: BAMBOO_PALETTES[stalk.paletteIndex].mid }} />
                            <div className="absolute w-[110%] h-[1px] left-[-5%] mt-[2px] rounded-full opacity-50" style={{ background: BAMBOO_PALETTES[stalk.paletteIndex].mid }} />
                            
                            {/* Branches */}
                            {joint.branches.map(branch => (
                              <motion.div
                                key={`branch-${branch.id}`}
                                initial={{ scaleY: 0, opacity: 0, rotate: branch.branchAngle }}
                                animate={{ scaleY: 1, opacity: 1, rotate: branch.branchAngle }}
                                transition={{ delay: jointDelay + 0.15, duration: 0.5, ease: 'easeOut' }}
                                className="absolute"
                                style={{
                                  width: '2px',
                                  height: `${branch.branchLength}px`,
                                  background: BAMBOO_PALETTES[stalk.paletteIndex].mid,
                              ...
                              >
                                ...
                                {branch.leaves.map(leaf => (
                                  <motion.div
                                    key={`leaf-${leaf.id}`}
                                    initial={{ scale: 0, opacity: 0, rotate: leaf.leafAngle }}
                                    animate={{ scale: 1, opacity: 1, rotate: leaf.leafAngle }}
                                    transition={{ delay: jointDelay + 0.4 + leaf.id * 0.07, duration: 0.4, ease: 'easeOut' }}
                                    className="absolute"
                                    style={{
                                      width: `${leaf.width}px`,
